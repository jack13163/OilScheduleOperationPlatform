...
public class MOEADD<S extends DoubleSolution> extends AbstractMOEAD<S> {
	...
	public MOEADD(Problem<S> problem, int populationSize, int resultPopulationSize, int maxEvaluations,
			CrossoverOperator<S> crossoverOperator, MutationOperator<S> mutation,
			AbstractMOEAD.FunctionType functionType, String dataDirectory, double neighborhoodSelectionProbability,
	...
	public void run() {
		...
		for (int i = 0; i < populationSize; i++) {
			double distance = calculateDistance2(population.get(i), lambda[i], idealPoint.getValues(),
					nadirPoint.getValues());
...
//        if (rankSolution.getOrDefault(indiv, 0) == (numRanks - 1)) {
			...
			if (frontSize == 1 && lastFront.get(0).equals(indiv)) { // the last non-domination level only has 'indiv'
			...
			} else if (frontSize == 1 && !lastFront.get(0).equals(indiv)) { // the last non-domination level only has
																			// one solution, but not 'indiv'
				...
				// find the index of the solution in the last non-domination level, and its
				...
				// find the most crowded subregion, if more than one exist, keep them in
				// 'crowdList'
				...
				case 0:
					...
					break;
				case 1:
					// if the subregion of each solution in the last non-domination level only has
					// one solution, keep them all
					deleteCrowdRegion2(indiv, location);
					break;
				default:
					// delete the worst solution from the most crowded subregion in the last
					...
					if (list.isEmpty()) {
						...
						if (idxArray[targetIdx] == -1) {
							...
							// int targetRank = ((DoubleSolution)
							// population.get(idxArray[targetIdx])).getRank();
							int targetRank = (int) population.get(idxArray[targetIdx])
									.getAttribute(ranking.getAttributeIdentifier());
	...
	public void nondominated_sorting_delete(S indiv) {
		...
		ArrayList<Integer> curLevel = new ArrayList<>(); // used to keep the solutions in the current non-domination
															// level
		...
		int flag;
		// find the solutions belonging to the 'indivRank+1'th level and are dominated
		// by 'indiv'
						...
						if (rankIdx[investigateRank][j] == 1) {
							...
							if (checkDominance(population.get(curIdx), population.get(j)) == 1) {
								...
								if (flag == 0) {
									...
									population.get(j).setAttribute(ranking.getAttributeIdentifier(),
											investigateRank - 1);
								}
							}
						}
	...
	public int nondominated_sorting_add(S indiv) {
		...
		for (int i = 0; i < num_ranks; i++) {
			level = i;
			if (flag == 1) { // 'indiv' is non-dominated with all solutions in the ith non-domination level,
								// then 'indiv' belongs to the ith level
				// indiv.setRank(i - 1);
				indiv.setAttribute(ranking.getAttributeIdentifier(), i - 1);
				return num_ranks;
			} else if (flag == 2) { // 'indiv' dominates some solutions in the ith level, but is non-dominated with
									// some others, then 'indiv' belongs to the ith level, and move the dominated
									// solutions to the next level
				// indiv.setRank(i - 1);
				indiv.setAttribute(ranking.getAttributeIdentifier(), i - 1);
				int prevRank = i - 1;

				// process the solutions belong to 'prevRank'th level and are dominated by
				// 'indiv' ==> move them to 'prevRank+1'th level and find the solutions
				// dominated by them
				int curIdx;
				...
				// if there are still some other solutions moved to the next level, check their
				// domination situation in their new level
				prevRank = newRank;
				newRank = newRank + 1;
				curListSize = dominateList.size();
				if (curListSize == 0) {
					...
					do {
						...
						if (curListSize != 0) {
							prevRank = newRank;
							newRank = newRank + 1;
							if (frontSize.size() > prevRank && curListSize == frontSize.get(prevRank)) { // if all
																											// solutions
																											// in the
																											// 'prevRank'th
																											// level are
			...
			} else if (flag == 3 || flag == 0) { // if 'indiv' is dominated by some solutions in the ith level, skip it,
													// and term to the next level
				flag1 = flag2 = flag3 = 0;
				for (int j = 0; j < populationSize; j++) {
					if (rankIdx[i][j] == 1) {
			...
			} else { // (flag == 4) if 'indiv' dominates all solutions in the ith level, solutions in
						// the current level and beyond move their current next levels
				// indiv.setRank(i - 1);
				indiv.setAttribute(ranking.getAttributeIdentifier(), i - 1);
				i = i - 1;
		...
		// if flag is still 3 after the for-loop, it means that 'indiv' is in the
		// current last level
		switch (flag) {
		case 1:
			// indiv.setRank(level);
	...
	/**
	 * Delete a solution from the most crowded subregion (this function only happens
	 * when: it should delete 'indiv' based on traditional method. However, the
	 * subregion of 'indiv' only has one solution, so it should be kept)
	 */
	public void deleteCrowdRegion1(S indiv, int location) {

		// find the most crowded subregion, if more than one such subregion exists, keep
		// them in the crowdList
		ArrayList<Integer> crowdList = new ArrayList<>();
		int crowdIdx;
	 ...
	 * when: it should delete the solution in the 'parentLocation' subregion, but
	 * since this subregion only has one solution, it should be kept)
	 */
	public void deleteCrowdRegion2(S indiv, int location) {

		double indivFitness = fitnessFunction(indiv, lambda[location]);

		// find the most crowded subregion, if there are more than one, keep them in
		// crowdList
		ArrayList<Integer> crowdList = new ArrayList<>();
		int crowdIdx;
		int nicheCount = countOnes(0);
	...
	public void deleteRankOne(S indiv, int location) {
		...
		case 0:
			...
			break;
		case 1:
			// if every subregion only contains one solution, delete the worst from indiv's
			// subregion
			int targetIdx;
		...
		default:
			if (location == crowdIdx) { // if indiv's subregion is the most crowded one
				...
				if (crowdNC > (curNC + 1)) { // if the crowdIdx subregion is more crowded, delete one from this
												// subregion
	 ...
	 * subregion. Compare indiv's fitness value and the worst one in this subregion
	 */
	...
	/**
	 * delete one solution from the most crowded subregion, which is different from
	 * indiv's subregion. just use indiv to replace the worst solution in that
	 * subregion
	 */
...
//        indiv.setAttribute(ATTRIBUTES.DIST, minDist);
	...
	/**
	 * check the dominance relationship between a and b: 1 -> a dominates b, -1 -> b
	 * dominates a 0 -> non-dominated with each other
	 */
