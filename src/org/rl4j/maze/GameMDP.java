...
public class GameMDP implements MDP<GameState, Integer, DiscreteSpace> {
    ...
    private boolean isOutOfBox() {
        if (curState.getX() < 0.0 || curState.getX() > 9.0 || curState.getY() < 0.0 || curState.getY() > 9.0) {
            return true;
        }
        return false;
    }
    ...
    public StepReply<GameState> step(Integer action) {
        ...
        switch (action) {
            case 0:
                curState.setX(curX - 1.0);
                break;//左
            case 1:
                curState.setX(curX + 1.0);
                break;//右
            case 2:
                curState.setY(curY - 1.0);
                break;//上
            case 3:
                curState.setY(curY + 1.0);
                break;//下
            default:
                throw new RuntimeException("Out of Action Space");
        }
