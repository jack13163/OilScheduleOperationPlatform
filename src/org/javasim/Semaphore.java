...
/**
 * Classic semaphores can "accumulate" more resources than the starting value.
 * The ceiling parameter is used to indicate whether or not the resource count
 * should ever go beyond the initial value - the default is that it should.
 */

public class Semaphore {
	/**
	 * The result of various operations: DONE - succeeded. NOTDONE - failed.
	 * WOULD_BLOCK - the operation would block the thread.
	 * 
	 * Note that for some operations only DONE and NOTDONE could be returned.
	 */

	public enum Outcome {
		DONE, NOTDONE, WOULD_BLOCK
	};
	 ...
	 * 
	 * @param toWait
	 *            the entity that will be blocked.
	 * @return an indication of the outcome (DONE, NOTDONE)
	 * @throws RestartException
	 *             if a reset occurs while an entity is blocked.
	 */

	public Outcome get(SimulationEntity toWait) throws RestartException {
	...
	public Outcome tryGet(SimulationEntity toWait) throws RestartException {
		if (currentResources == 0)
