...
public class SimulationProcess extends Thread {
	 ...
	 *             thrown if there's an error.
	 * @throws NoSuchElementException
	 *             thrown if there is no next processs.
	 */
	 ...
	 *             thrown if the simulation is restarted.
	 */
	 ...
	 * 
	 * @param AtTime
	 *            the time to activate the process.
	 * @param prior
	 *            indicates whether or not to schedule this process occurs
	 *            before any other process with the same time.
	 * @throws SimulationException
	 *             thrown if there's an error.
	 * @throws RestartException
	 *             thrown if the simulation is restarted.
	...
	public void activateAt(double AtTime, boolean prior) throws SimulationException, RestartException {
		...
		if (AtTime < SimulationProcess.currentTime())// ʱ�䲻�Ϸ�
	 ...
	 * 
	 * @param AtTime
	 *            the time to activate this process.
	 * @throws SimulationException
	 *             thrown if there's an error.
	 * @throws RestartException
	 *             thrown if the simulation is restarted.
	 ...
	 * 
	 * @param Delay
	 *            the time by which to delay this process.
	 * @param prior
	 *            indicates whether or not to schedule this process occurs
	 *            before any other process with the same time.
	 * @throws SimulationException
	 ...
	 * 
	 * @param p
	 *            the process to reactivate this process before.
	 * @throws SimulationException
	 *             thrown if there's an error.
	 * @throws RestartException
	 *             thrown if the simulation is restarted.
	 ...
	 * 
	 * @param p
	 *            the process to reactivate this process after.
	 * @throws SimulationException
	 *             thrown if there's an error.
	 * @throws RestartException
	 *             thrown if the simulation is restarted.
	 ...
	 * 
	 * @param AtTime
	 *            the time at which to reactivate this process.
	 * @param prior
	 *            indicates whether or not to schedule this process occurs
	 *            before any other process with the same time.
	 * @throws SimulationException
	 ...
	 * 
	 * @param Delay
	 *            the time to delay this process by before reactivation.
	 * @param prior
	 *            prior indicates whether or not to schedule this process occurs
	 *            before any other process with the same time.
	 * @throws SimulationException
	 *             thrown if there's an error.
	 * @throws RestartException
	 ...
	 * 
	 * @param Delay
	 *            the time to delay this process.
	 * @throws SimulationException
	 *             thrown if there's an error.
	 * @throws RestartException
	 *             thrown if the simulation is restarted.
	...
	public void terminate() {
		if (!terminated) {
			...
			if ((this != SimulationProcess.Current) && (!idle()))
				Scheduler.unschedule(this);// ��Scheduler��ɾ��ģ��
	...
	public void mainSuspend() {
		SimulationProcess.mainThread = Thread.currentThread();

		try {

			// make sure main thread suspend
			if (suspendMainThread.availablePermits() == 0) {
				suspendMainThread.acquire();
			} else {
				System.out.println("main suspend error.");
				System.exit(1);
			}
	...
	/**
	 * if the main thread suspend ?
	 */

	public boolean mainSuspended() {

		if (suspendMainThread.hasQueuedThreads()) {
			return true;
		} else {
			return false;
		}
	...
	public void mainResume() throws SimulationException {
		if (SimulationProcess.mainThread == null)
			throw new SimulationException("No main thread");

		// make sure main thread resume
		if (suspendMainThread.hasQueuedThreads()) {
			suspendMainThread.release();
		} else {
			System.out.println("main resume error.");
			System.exit(1);
		}
	}
	...
	protected void resumeProcess() {
		...
		if (!terminated) {
			if (!started) {
				started = true;
				cachedThreadPool.execute(this);// 【此处为同一创建线程的地方，这里该用线程池的方式】start();
			} else {
				oneSignal.release();
			}
		}
	...
	private Semaphore oneSignal = new Semaphore(0);// 控制一次只能运行一个仿真对象

	private static Semaphore suspendMainThread = new Semaphore(0);
