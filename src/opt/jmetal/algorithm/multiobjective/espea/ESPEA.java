...
public class ESPEA<S extends Solution<?>> extends AbstractGeneticAlgorithm<S, List<S>> {
    ...
    /**
     * ESPEA uses two different crossover operators depending on the current archive
     * size. If the archive is not full, it uses the crossover operator provided by
     * {@link #getCrossoverOperator()}. If the archive is full,
    ...
    public ESPEA(Problem<S> problem, int maxEvaluations, int populationSize, CrossoverOperator<S> crossoverOperator,
                 ...
                 SelectionOperator<List<S>, S> selectionOperator, ScalarizationWrapper scalarizationWrapper,
                 SolutionListEvaluator<S> evaluator, boolean normalizeObjectives, EnergyArchive.ReplacementStrategy replacementStrategy) {
        super(problem);
        ...
        this.archive = new EnergyArchive<>(populationSize, scalarizationWrapper, normalizeObjectives,
                replacementStrategy);
     ...
     *
     * @see org.uma.jmetal.algorithm.impl.AbstractGeneticAlgorithm#selection(java.
     * util.List)
     */
    @Override
