...
import opt.jmetal.algorithm.multiobjective.wasfga.util.WeightVectors;
import opt.jmetal.algorithm.InteractiveAlgorithm;
import opt.jmetal.algorithm.multiobjective.mombi.AbstractMOMBI;
import opt.jmetal.algorithm.multiobjective.mombi.util.ASFWASFGA;
import opt.jmetal.algorithm.multiobjective.mombi.util.AbstractUtilityFunctionsSet;
import opt.jmetal.algorithm.multiobjective.wasfga.util.WASFGARanking;
 ...
 * Issue 1, pp 101-129 DOI = {10.1007/s10898-014-0214-y}
 */
public class WASFGA<S extends Solution<?>> extends AbstractMOMBI<S> implements InteractiveAlgorithm<S, List<S>> {
     ...
     */
    public WASFGA(Problem<S> problem, int populationSize, int maxIterations, CrossoverOperator<S> crossoverOperator,
                  MutationOperator<S> mutationOperator, SelectionOperator<List<S>, S> selectionOperator,
                  SolutionListEvaluator<S> evaluator, double epsilon, List<Double> referencePoint,
    ...
    public WASFGA(Problem<S> problem, int populationSize, int maxIterations, CrossoverOperator<S> crossoverOperator,
        ...
        this(problem, populationSize, maxIterations, crossoverOperator, mutationOperator, selectionOperator, evaluator,
                epsilon, referencePoint, "");
    }

    public AbstractUtilityFunctionsSet<S> createUtilityFunction() {
        // If a file with weight vectors is not given as parameter, weights are
        // calculated or read from the resources file of jMetal
        if ("".equals(this.weightVectorsFileName)) {
            ...
            // For more than two objectives, weights are read from the resources file of
            // jMetal
            else {
        ...
        } else { // If a file with weight vectors is given as parameter, weights are read from
            // that file
        ...
        // The number of components of each weight is similar to the number of
        // objectives of the problem being solved.
        ...
        // By default, the algorithm uses as many weight vectors as individual in the
        // population.
        // In a future, a new parameter should be added to specify the number of weight
        // vectors to use.
        // As it is mentioned in the paper, the number of weight vectors must lower than
