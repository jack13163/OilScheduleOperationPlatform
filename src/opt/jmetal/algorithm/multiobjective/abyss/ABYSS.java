...
import opt.jmetal.algorithm.multiobjective.abyss.util.MarkAttribute;
import opt.jmetal.algorithm.impl.AbstractScatterSearch;
...
import opt.jmetal.util.comparator.StrengthFitnessComparator;
import opt.jmetal.util.pseudorandom.JMetalRandom;
import opt.jmetal.util.solutionattribute.impl.DistanceToSolutionListAttribute;
import opt.jmetal.util.solutionattribute.impl.StrengthRawFitness;

/**
 * This class implements the AbYSS algorithm, a multiobjective scatter search
 * metaheuristics, which is described in: A.J. Nebro, F. Luna, E. Alba, B.
 * Dorronsoro, J.J. Durillo, A. Beham "AbYSS: Adapting Scatter Search to
 * Multiobjective Optimization." IEEE Transactions on Evolutionary Computation.
 * Vol. 12, No. 4 (August 2008), pp. 439-457
 *
...
public class ABYSS extends AbstractScatterSearch<DoubleSolution, List<DoubleSolution>> {
    ...
    public ABYSS(DoubleProblem problem, int maxEvaluations, int populationSize, int referenceSet1Size,
                 ...
                 LocalSearchOperator<DoubleSolution> localSearch, CrossoverOperator<DoubleSolution> crossoverOperator,
    ...
    public DoubleSolution diversificationGeneration() {
        ...
        for (int i = 0; i < problem.getNumberOfVariables(); i++) {
            ...
            double low = ((DoubleProblem) problem).getLowerBound(i)
                    + range * (((DoubleProblem) problem).getUpperBound(i) - ((DoubleProblem) problem).getLowerBound(i))
                    / numberOfSubRanges;
            double high = low
                    + (((DoubleProblem) problem).getUpperBound(i) - ((DoubleProblem) problem).getLowerBound(i))
                    / numberOfSubRanges;
    ...
    /**
     * Build the referenceSet1 by moving the best referenceSet1Size individuals,
     * according to a fitness comparator, from the population to the referenceSet1
     */
    ...
    /**
     * Build the referenceSet2 by moving to it the most diverse referenceSet2Size
     * individuals from the population in respect to the referenceSet1.
     * <p>
     * The size of the referenceSet2 can be lower than referenceSet2Size depending
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < referenceSet2.size(); j++) {
                for (int k = 0; k < referenceSet2.size(); k++) {
                    if (i != j) {
                        double aux = SolutionUtils.distanceBetweenSolutionsInObjectiveSpace(referenceSet2.get(j),
                                referenceSet2.get(k));
