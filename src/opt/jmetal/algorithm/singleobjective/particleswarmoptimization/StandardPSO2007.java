package opt.jmetal.algorithm.singleobjective.particleswarmoptimization;
...
import opt.jmetal.operator.Operator;
import opt.jmetal.operator.impl.selection.BestSolutionSelection;
...
import opt.jmetal.util.neighborhood.impl.AdaptiveRandomNeighborhood;
...
public class StandardPSO2007 extends AbstractParticleSwarmOptimization<DoubleSolution, DoubleSolution> {
     ...
     *
     * @param problem
     * @param objectiveId               This field indicates which objective, in the
     *                                  case of a multi-objective problem, is
     ...
     * @param evaluator
     */
    public StandardPSO2007(DoubleProblem problem, int swarmSize, int maxIterations, int numberOfParticlesToInform,
    ...
    public void initializeVelocity(List<DoubleSolution> swarm) {
        for (int i = 0; i < swarm.size(); i++) {
            ...
            for (int j = 0; j < problem.getNumberOfVariables(); j++) {
                speed[i][j] = (randomGenerator.nextDouble(particle.getLowerBound(j), particle.getUpperBound(j))
