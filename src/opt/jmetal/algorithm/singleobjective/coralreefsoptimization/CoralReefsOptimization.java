package opt.jmetal.algorithm.singleobjective.coralreefsoptimization;
...
import opt.jmetal.algorithm.impl.AbstractCoralReefsOptimization;
...
import opt.jmetal.util.pseudorandom.impl.MersenneTwisterGenerator;
...
public class CoralReefsOptimization<S> extends AbstractCoralReefsOptimization<S, List<S>> {
    ...
    public CoralReefsOptimization(Problem<S> problem, int maxEvaluations, Comparator<S> comparator,
        ...
        super(comparator, selectionOperator, crossoverOperator, mutationOperator, n, m, rho, fbs, fa, pd,
                attemptsToSettle);
    ...
    protected List<Coordinate> generateCoordinates() {
        ...
        for (int i = 0; i < popSize; i++) {
            coordinates.add(new Coordinate(random.nextInt(0, getN() - 1), random.nextInt(0, getM() - 1)));
        }
    ...
    protected List<S> larvaeSettlementPhase(List<S> larvae, List<S> population, List<Coordinate> coordinates) {
        ...
        for (S larva : larvae) {
            ...
            for (int attempt = 0; attempt < attempts; attempt++) {
                Coordinate C = new Coordinate(random.nextInt(0, getN() - 1), random.nextInt(0, getM() - 1));
