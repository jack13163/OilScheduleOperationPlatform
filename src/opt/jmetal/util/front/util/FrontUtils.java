...
import opt.jmetal.util.point.util.distance.EuclideanDistance;
import opt.jmetal.util.point.util.distance.PointDistance;
...
public class FrontUtils {
    ...
    /**
     * Gets the distance between a point and the nearest one in a front. If a
     * distance equals to 0 is found, that means that the point is in the front, so
     * it is excluded
     *
     ...
     * if (front == null) { throw new JMetalException("The front is null"); }
     *
     * int numberOfObjectives ; int solutionSetSize = front.getNumberOfPoints() ; if
     * (front.getNumberOfPoints() == 0) { numberOfObjectives = 0 ; } else {
     * numberOfObjectives = front.getPoint(0).getDimension(); } List<PointSolution>
     * solutionSet = new ArrayList<>(solutionSetSize) ;
     *
     * for (int i = 0; i < front.getNumberOfPoints(); i++) { PointSolution solution
     * = new PointSolution(numberOfObjectives); for (int j = 0 ; j <
     * numberOfObjectives; j++) { solution.setObjective(j,
     * front.getPoint(i).getValue(j)); }
     *
