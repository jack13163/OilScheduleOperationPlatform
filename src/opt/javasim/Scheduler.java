...
public class Scheduler extends Thread {
    ...
    /**
     * This routine resets the simulation time to zero and removes all entries from
     * the scheduler queue (as their times may no longer be valid). Whatever
     * operation caused the processes to become suspended will raise the
     * RestartSimulation exception, which the application should catch. It should
     * then perform any work necessary to put the process back in a state ready for
     * restarting the simulation before calling Cancel on the process.
     *
    ...
    static synchronized void reset() throws SimulationException {
        ...
        do {
            try {
                ...
                /*
                 * Every process must be in Suspend, so we call Resume and get each one to check
                 * whether the simulation is restarting. If it is, it raises an exception and
                 * waits for the user to cancel the process after setting it to become ready to
                 * restart.
                 */
     ...
     * simulation queue. In which case we don't want to suspend the process since it
     * needs to continue to run. The return value indicates whether or not to call
    ...
    static synchronized boolean schedule() throws SimulationException {
        if (Simulation.isStarted()) {
            ...
            if (SimulationProcess.Current.evtime() < 0)
                ...
                Scheduler.SimulatedTime = MathHelper.precision(SimulationProcess.Current.evtime(), 2);// 保留精度
