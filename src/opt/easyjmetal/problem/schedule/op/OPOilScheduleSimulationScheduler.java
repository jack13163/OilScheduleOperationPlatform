...
public class OPOilScheduleSimulationScheduler implements ISimulationScheduler {
     ...
     * 开始
     */
    public void start(Solution solution) throws JMException {
        // 清空决策队列
        getOperations().clear();
        fragmentStack.clear();
     ...
     * @return
     */
    private boolean process() throws JMException {
        // 最长决策指令序列长度
     ...
     * 多次回溯不可行后，后退若干步
     */
    private void superBackTrace() throws JMException {
        int k = 3;
     ...
     * @return
     */
    private void last() throws JMException {
        backTimes++;
     ...
     */
    private void retryCurrentFragment(Fragment currentFragment) throws JMException {
        ...
        if (currentTank != 0 && currentSpeed != maxSpeed && currentVolume >= config.VolMin) {
            ...
            variables[loc * 2 + 1].setValue(0.99);
