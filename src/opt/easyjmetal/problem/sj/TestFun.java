...
public class TestFun {
    ...
    public static double getPipeStopTime(int[][] FPORDER,double[] DSFR,int RT,double[][] TKS, double PET, double[] FP, List<Integer> UD){
        ...
        sort(UFDR,new int[]{1});
        double[][] available=new double[DSFR.length][2]; //库存中各个蒸馏塔可用油量

        for(int i=0;i<UD.size();i++){  //计算炼油场内各个蒸馏塔可用原油量
            int DSN=(int)UFDR[i][0]; //蒸馏塔
            int COTN1=FPORDER[DSN-1][0]; //原油类型1
            int COTN2=FPORDER[DSN-1][1];//原有类型2
        ...
        double[] total=new double[FPORDER.length];
        ...
        int K=UFDR.length;
        for(int i=0;i<K;i++){
            double arfai=RT*UFDR[i][1]; //静置时间需要的原油量
            int DSN=(int)UFDR[i][0]; //蒸馏塔序号

            //预留一个调度周期的原油
            if(i==K-1){
                total[DSN-1]=total[DSN-1]-2*K*arfai;
            }
            else{
                total[DSN-1]=total[DSN-1]-K*arfai;
            }

            int  tmp=(int)(total[DSN-1]/UFDR[i][1]);
            if(tmp<t){
                t=tmp;
            }
        }
