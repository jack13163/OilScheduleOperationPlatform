				...
				Thread mainThread = new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							logger.fatal(e.getMessage() + "，多迭代几次试试");
						}
						// 取消禁用UI
						enabledUI();
					}
			...
			public void actionPerformed(ActionEvent e) {
				...
				Thread mainThread = new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							// 获取问题列表
							List<ExperimentProblem<DoubleSolution>> problems = ExperimentConfig
									.getTestProblemsList(problemList, 50, 4);
							ExperimentConfig.doTestExperiment(problems, algorithmList, popSize, evaluation, runs);
