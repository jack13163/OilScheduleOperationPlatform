...
import oil.rules.AbstractRule;
import oil.sim.common.ArrayHelper;
import oil.sim.common.CodeHelper;
import oil.sim.oil.Config;
import oil.sim.common.ISimulationScheduler;
import oil.sim.oil.cop.COPOilScheduleSimulationScheduler;
 ...
 */
public class EarliestDealineFirstAndTwoStepsScheduling extends AbstractRule {

    public EarliestDealineFirstAndTwoStepsScheduling(ISimulationScheduler scheduler) {
        super(scheduler);
    }
    ...
    public Fragment decode(FactObject factObject) {
        ...
        // 3.若高熔点管道未完成炼油计划，则转运高熔点原油
        if (config.getDSs().get(config.HighOilDS - 1).getNextOilVolume() > 0) {
            for (int i = 0; i < vols.length; i++) {
                if (config.HighOilDS != i + 1) {
                    for (int j = 0; j < vols[i].length; j++) {
                        // 未完成高熔点原油转运计划之前，不进行低熔点原油转运
                        policies[i][j] = 0;
                    }
                }
            }
        ...
        try {
            ...
            // 4.2 确定供油罐【从中间向两边搜索距离最近的策略，左边优先搜索】
